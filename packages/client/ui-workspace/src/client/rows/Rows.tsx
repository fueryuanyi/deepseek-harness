...
export function SessionNodeItem({
  node, currentId, now, onOpen, onRename, onFork, onArchive, onUnarchive, onDelete,
  ...
  /** Unarchive this session (row menu action; archived rows only). */
  onUnarchive?: (id: SessionNode['id']) => void
  /** Delete this session durably (row menu action; archived rows only, destructive). */
  onDelete?: (id: SessionNode['id']) => void
  /** The row belongs to the Archived section (archive is replaced by unarchive). */
  ...
  const sessionMenuItems = [
    ...
    ...(archived && onDelete !== undefined
      ? [{ id: 'delete' as const, label: t('menu.deleteSession'), icon: <IconTrashOutline16 />, danger: true }]
      : []),
  ]
