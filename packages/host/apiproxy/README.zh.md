...
`session.delete` 永久移除一个会话：先把会话从注册表级全局归档集合和每个 workspace 记账中清除（`workspaceRegistry.removeSession`），再由持久化层删除持久日志，因此部分失败时该会话会退化为 Ungrouped，而不是留下悬空记账槽。活动（已附加）会话以 `session-busy` 失败；调用方必须先停止它。已不存在的 id 幂等地直接完成。
