...
| `delete(id): Promise<boolean>` | Durably remove one session's log and in-memory state. A live (attached) session rejects so in-flight writes are never orphaned; the coordinator invalidates any retained cold view before the backend removes storage. Returns `true` when the identity was removed, `false` when it was absent (idempotent). |
...
| `deleteStored(id)` | Durably remove one session's storage (artifact or rows). The coordinator rejects live sessions before this hook runs, so a backend only ever deletes cold storage; returns `false` when the identity is absent. |
