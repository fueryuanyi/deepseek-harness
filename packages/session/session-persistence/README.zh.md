...
| `delete(id): Promise<boolean>` | 持久删除一份会话日志及其内存状态。活动（已附加）会话会被拒绝，以免丢弃正在进行的写入；协调器在后端删除存储前会先使失效任何保留的冷视图。身份被删除时返回 `true`，身份不存在时返回 `false`（幂等）。 |
...
| `deleteStored(id)` | 持久删除一份会话的存储（工件或行）。协调器会先拒绝活动会话，因此后端只删除冷存储；身份不存在时返回 `false`。 |
