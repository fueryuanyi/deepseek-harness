...
- `ctx.workspaceRegistry.unarchiveSession(id)` — removes a session from the registry-global archive set, restoring its grouping-surface slot (its workspace account or the Ungrouped bucket). An id not in the set resolves without writing; the idempotent skip keeps a lost unarchive retry from corrupting the set.
