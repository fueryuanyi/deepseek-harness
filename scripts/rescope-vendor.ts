...
const GENERIC_SKIPS: readonly GenericSkip[] = [
  ...
  // `cordis` is also the dynamic-plugin event scope (`cordis/*` event names) and
  // the EVENT_SCOPE_PAGE key — a product-domain identifier, not the vendored
  // package. These files reference that scope, never an un-scoped package name.
  ...
  // The `cordis` UI locale namespace (NS = 'cordis', PropsLocale<'cordis'>,
  // t('cordis')) is product data, not a package reference.
